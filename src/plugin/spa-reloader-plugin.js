const methods = {
    ...
    $webSocketsConnect (options) {
        ...
        ws.onclose = (event) => {
            console.log("Closing Connection to SPA Relaoad at: ", options.spaReloaderURL);
            if (event) {
...
export default {
    ...
    install(Vue, options = {}) {
        ...
        // Validate options
        if (!spaReloaderURL) throw new Error('Invalid SPA Reloader URL, please supply the URL where SPA Reloader is running.')
